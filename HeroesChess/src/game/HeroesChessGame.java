...
//			Speak.say("\n-1 for testing whether start/end space is empty.");
			...
			while(!(posOfPieceToMove > -1 && posOfPieceToMove < 64))
				...
				if(!moveList.isEmpty()) {
					...
					if(selectedMove >= 0) {
						...
						if(!moves.testCheck(playerTurn, moveBits)) {				
							...
							Speak.say("piece moved: " + typeToMove, true);
							...
							if(Character.toUpperCase(typeToMove) == 'K') {
								...
								// if king move was a castling, also move rook
							...
							if(Character.toUpperCase(typeToMove) == 'R')
								if(posOfPieceToMove == 0 || posOfPieceToMove == 56)
									kSideCastling[playerTurn] = false;
								else if(posOfPieceToMove == 7 || posOfPieceToMove == 63)
